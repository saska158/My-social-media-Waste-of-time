...
const CommentActions = ({roomId=null, room, postId, commentId, showReplies, setShowReplies, numberOfReplies, setNumberOfReplies}) => {
  ...
  const handleLike = async (e) => {
    ...
    if(!user) {
      setIsJoinPopupShown(true)
       ...
       const commentRef = doc(firestore, room, postId, 'comments', commentId)
       try {
          const snapshot = await getDoc(commentRef)
          const likes = snapshot.data().likes || {}

          if (likes[user?.uid]) {
            await updateDoc(commentRef, {[`likes.${user.uid}`]: deleteField()})
            console.log("Unliked")
          } else {
            await updateDoc(commentRef, {[`likes.${user.uid}`]: {name: user.displayName}})
            console.log("Liked")
          }
       } catch (error) {
  ...
  useEffect(() => {
    const repliesRef = collection(firestore, room, postId, 'comments', commentId, 'replies')
    
    const unsubscribe = onSnapshot(repliesRef, (snapshot) => { 
      if(!snapshot.empty) {
        setNumberOfReplies(snapshot.size)
      } else {
  ...
  return (
    <div className="post-actions">
      <div className="post-actions-container">
        ...
        <span>{numberOfReplies} {numberOfReplies === 1 ? 'reply' : 'replies'}</span>
      </div>
      <div>
