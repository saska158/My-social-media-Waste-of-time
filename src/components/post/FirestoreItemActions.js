 ...
 /* const profileImgsArray = likesArray.map(like => like.photoURL)
  ...
  return (
    <div className="post-actions">
        {
          likesArray.length > 0 && type !== 'comment' && (
            <div 
              style={{ 
                display: 'flex', 
                alignItems: 'center', 
            ...
            >
              <div 
                ...
                style={{ width: `${17 * (likesArray.slice(0, 3).length - 1)}px` }}
              >
                {
                  likesArray.length > 0 &&
              ...
              <div
                ...
                style={{display: 'flex', alignItems: 'center', gap: '.3em', cursor: 'pointer', fontWeight: '500'}}
              >
                <span>Liked by</span>
                <span style={{fontWeight: '800'}}>{likesArray[0]?.displayName}</span>
                {
                  likesArray.length === 2 && (
                    <p>and <span style={{fontWeight: '800'}}>1 other</span></p>
                  )}
                {
                  likesArray.length > 2 && (
                    <p>and <span style={{fontWeight: '800'}}>{likesArray.length - 1} others</span></p>
                  )}
              </div>
            </div>
          )
        ...
        <div className="post-actions-buttons">
          <button onClick={handleLike} style={{display: 'flex', alignItems: 'center'}}>
            {
              isLiked ? (
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6" style={{width: '30px'}}>
            ...
            {
              type === 'comment' && (
                <span style={{fontSize: '1rem'}}>{likesArray.length}</span>
              )
            }
          </button>
          <button onClick={handleShowComments} style={{display: 'flex', alignItems: 'center', gap: '.3em'}}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6" style={{width: '30px'}}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a5.969 5.969 0 0 1-.474-.065 4.48 4.48 0 0 0 .978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z" />
            </svg>
            <span style={{fontSize: '1rem'}}>{numberOfComments}</span>
          </button>
        </div>
